...
export function createOtp(userId: string): string {
  ...
  ).run(userId, sha256(code), isoPlus(OTP_TTL_MS));
  return code;
}
...
export function verifyOtp(userId: string, code: string): VerifyResult {
  const row = db
    .prepare("SELECT * FROM password_otps WHERE user_id = ?")
  ...
  db.prepare(
    `UPDATE password_otps
     ...
     WHERE user_id = ?`
  ).run(sha256(resetToken), isoPlus(RESET_TTL_MS), userId);

  return { ok: true, resetToken };
}

// Validate a reset token issued by verifyOtp. Returns the owning user, or null.
export function consumeResetToken(rawToken: string): DbUser | null {
  const row = db
    .prepare(
      "SELECT user_id, reset_expires_at FROM password_otps WHERE reset_token_hash = ?"
    )
    .get(sha256(rawToken)) as
    | { user_id: string; reset_expires_at: string | null }
    | undefined;

  if (!row) return null;
  if (isExpired(row.reset_expires_at)) {
    db.prepare("DELETE FROM password_otps WHERE user_id = ?").run(row.user_id);
    return null;
  }
